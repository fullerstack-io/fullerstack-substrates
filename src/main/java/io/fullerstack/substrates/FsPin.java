...
public final class FsPin < E > implements Pin < E > {
  ...
  /// against the pin's owning circuit's `current()`". [FsCircuit#onWorker] is that same
  /// question in the form §11.3 names as the Java mechanism — §5.7 requires this circuit's
  /// `Current` to BE its worker's, so the tokens compare equal exactly when the threads do.
  ///
  /// It is asked that way because the literal form is not free, and this doc used to claim it
  /// was: "JIT-hoisted out of hot loops, so the per-access cost converges to a plain field
  /// read". The suite says otherwise — `PinOps.get_loop_owner_context` measured 5.381 ns
  /// against `baseline_field_get_loop` at 0.064 ns, so the guard was 84x the read it guards.
  /// `FsCircuit.onWorker`'s own note explains why: `cortex().current()` is six dependent loads
  /// through a ThreadLocal, measured at 16.9% of the emission profile when it sat there.
  ///
  /// The `Current` is still used where it earns its cost: on the **cold** fault path, so the
  /// error names the offending context rather than describing it in prose (§15.3).
